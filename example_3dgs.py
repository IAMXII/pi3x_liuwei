...
# ==========================================
# 1. Custom Intrinsics Loader (关键修改)
# ==========================================
def load_custom_intrinsics(device='cuda', H=None, W=None):
    """
    [TODO]: 在这里修改代码以从你的脚本读取内参
    返回:
        K (torch.Tensor): 形状为 [3, 3] 或 [1, 3, 3] 的内参矩阵
    """
    print("Loading intrinsics from custom script...")

    # --- 示例：假设你的脚本叫 my_cam_params.py，里面有个变量 K ---
    # from my_cam_params import K as K_numpy
    # K_tensor = torch.from_numpy(K_numpy).to(device).float()
    ...
    # [占位符] 如果你还没有替换上面的代码，这里会抛出错误提醒你
    raise NotImplementedError("请在 load_custom_intrinsics 函数中实现你的内参读取逻辑！")

    # 确保返回的是 [1, 3, 3] 形状以便后续广播
    if K_tensor.ndim == 2:
