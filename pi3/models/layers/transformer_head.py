...
class AnchorGaussianHead(nn.Module):
    ...
    def forward(self, tokens, camera_poses, intrinsics, img_shape, selected_anchors,
        ...
        for v in range(N_views):
            pose = camera_poses[:, v]
            K_mat = intrinsics[:, v]  # 获取当前视角的内参 [B, 3, 3]
            ...
            # 【注意】建议使用 padding_mode='zeros'，防止越界点污染特征
        ...
        # =========================================================
        # [MEMORY OPTIMIZATION & GRADIENT FLOW] 软硬掩码分离
        # =========================================================
        
        # 准备缩放和切分概率（base anchor 也就是索引 0 的点恒为 1.0）
        ...
        if self.training:
            ...
            # 3. 告诉外层 Loss 和渲染器：本 Batch 所有生成的点都要参与渲染
            # gsplat 的底层光栅化会自动剔除 opacity < epsilon (通常是1/255) 的无效点
            num_near = torch.full((B,), flat_xyz.shape[1], dtype=torch.long, device=slots.device)
            ...
            # --- 推理阶段：硬剪枝 (Hard Pruning) ---
            # 保持你原本的逻辑，利用 argsort 彻底丢弃低概率点，极致节省显存
