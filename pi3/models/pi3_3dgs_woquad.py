...
class Pi3_3DGS(nn.Module):
    ...
    def _load_state_dict_flexible(self, checkpoint):
        ...
        if skipped:
            ...
            print(f"[Pi3_3DGS_woquad] Skipped incompatible checkpoint tensors: {preview}{suffix}")
        return res
    ...
    def forward(self, imgs, intrinsics=None, chunk_size=30000, global_step=None, return_viz=False):
        ...
        # ==========================================================
        # 2. 无四叉树候选：保留每个输入视角的全分辨率候选点
        # ==========================================================
        with torch.no_grad():
            keep_mask = torch.ones((B, N_sub, H, W), dtype=torch.bool, device=imgs.device)
            scale_map = torch.ones((B, N_sub, H, W, 1), dtype=torch.float32, device=imgs.device)
            quad_keep_mask = torch.zeros((B, N_sub, H, W, 1), dtype=torch.bool, device=imgs.device)
            if return_viz:
                empty_score = torch.zeros((B, N_sub, H, W), dtype=imgs.dtype, device=imgs.device)
                score_map_viz = empty_score.detach().clone()
                color_score_viz = empty_score.detach().clone()
                depth_score_viz = empty_score.detach().clone()

        # --- 提取并应用属性：先筛点，再分块解码 GS 属性 ---
        viz_outputs = None
        if return_viz:
            viz_outputs = {
        ...
        #####
            ...
            for start in range(0, N_sub, self.gs_decoder_view_chunk_size):
                ...
                # 1. 提取当前 chunk 内有效点：dense proposal + optional learned density extras.
