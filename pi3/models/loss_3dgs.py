...
class Pi3LossGS(nn.Module):
    def __init__(
            ...
            lambda_sparsity=0.005,
            lambda_consist=0.05,
            # [新增] Camera Pose Loss 权重
            lambda_pose=0.1,  
            # [新增] 默认开启，但可以在调用时通过权重设为 0 关闭
            warmup_ratio=0.3
    ):
        super().__init__()
        self.lambda_rgb = lambda_rgb
        ...
        self.lambda_sparsity = lambda_sparsity
        self.lambda_consist = lambda_consist
        self.lambda_pose = lambda_pose # 保存 pose 权重
    ...
    def prepare_gt(self, gt):
        ...
        # 1. 坐标系对齐 (将第0帧设为世界坐标原点)
        w2c_target = se3_inverse(poses_c2w[:, 0])
        gt_pts = torch.einsum('bij, bnhwj -> bnhwi', w2c_target, homogenize_points(gt_pts))[..., :3]
        # 计算相对位姿 (Ground Truth 的 C2W)
        poses_rel_c2w = torch.einsum('bij, bnjk -> bnik', w2c_target, poses_c2w)
        ...
        return dict(
            imgs=gt_imgs,
            render_w2c=render_w2c,      # 渲染用 W2C
            gt_c2w=poses_rel_c2w,       # [新增] 监督用 C2W (Ground Truth)
            render_ks=gt_ks,
            gt_depths=gt_depths,
            norm_factor=norm_factor
        )
    ...
    def forward(self, pred, gt_raw, current_epoch=None, total_epochs=None, enable_pose_loss=False):
        """
        Args:
            enable_pose_loss: [新增] 开关，可以在验证/推理时强制关闭，或者训练时手动传入
        """
        # 1. 准备 GT
        gt = self.prepare_gt(gt_raw)
        ...
        render_w2c = gt['render_w2c']
        render_ks = gt['render_ks']
        gt_c2w = gt['gt_c2w'] # [新增] 获取 GT C2W

        gauss = pred['gaussians']
        pred_c2w = pred['camera_poses'] # [新增] 获取预测的 C2W [B, N, 4, 4]
        num_near = gauss.get('num_near', None) 
        ...
        if self.lambda_pose > 0 and enable_pose_loss:
            # 旋转矩阵的 L1 Loss
            flip_yz = torch.eye(4, device=gt_c2w.device)
            flip_yz[1, 1] = -1
            flip_yz[2, 2] = -1

            # 尝试方案 A: 翻转 GT
            gt_c2w_flipped = torch.matmul(gt_c2w, flip_yz) 
            loss_pose_R = F.l1_loss(pred_c2w[..., :3, :3], gt_c2w_flipped[..., :3, :3])
            # loss_pose_R = F.l1_loss(pred_c2w[..., :3, :3], gt_c2w[..., :3, :3])
            # 平移向量的 L1 Loss (场景已经归一化，尺度可比)
        ...
        # 7. 汇总 Loss
        # Pose Loss 通常由 R 和 T 两部分组成，权重设为 self.lambda_pose
        ...
        details = {
            ...
            "loss_gs_ssim": loss_ssim,
            "loss_gs_depth": loss_depth,
            "loss_pose_R": loss_pose_R, # 记录旋转损失
            "loss_pose_T": loss_pose_T, # 记录平移损失
            "loss_repulsion": loss_repulsion,
            "loss_sparsity": loss_sparsity,
            "loss_consist": loss_consist,
            "total_loss": final_loss,
