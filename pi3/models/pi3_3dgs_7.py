...
class Pi3_3DGS(nn.Module):
    def __init__(
        ...
        self.dense_head_view_chunk_size = max(1, int(dense_head_view_chunk_size))
    ...
    def _run_conv_head_chunked(self, head, tokens, patch_h, patch_w):
        chunk_size = self.dense_head_view_chunk_size
        if tokens.shape[0] <= chunk_size:
            return head(tokens, patch_h=patch_h, patch_w=patch_w)
        ...
        for start in range(0, tokens.shape[0], chunk_size):
            ...
            chunk_out = head(tokens[start:end], patch_h=patch_h, patch_w=patch_w)
            ...
            if output_parts is None:
                output_parts = [[] for _ in chunk_seq]
            for out_idx, tensor in enumerate(chunk_seq):
                output_parts[out_idx].append(tensor)

        outputs = [torch.cat(parts, dim=0) for parts in output_parts]
        if output_is_tuple:
    ...
    def forward(self, imgs, intrinsics=None, chunk_size=30000, global_step=None, return_viz=False):
        ...
        with point_ctx:
            ...
            local_xyz_raw = self._run_conv_head_chunked(self.point_head, point_h, patch_h, patch_w)
        ...
        with conf_ctx:
            ...
            conf = self._run_conv_head_chunked(
                self.conf_head,
