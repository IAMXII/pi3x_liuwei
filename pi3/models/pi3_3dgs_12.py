...
class Pi3_3DGS(nn.Module):
    def __init__(
        ...
        patch_sizes = sorted({max(1, int(size)) for size in quadtree_patch_sizes}, reverse=True)
        if 1 not in patch_sizes:
            patch_sizes.append(1)
        self.quadtree_patch_sizes = patch_sizes
    ...
    def forward(self, imgs, intrinsics=None, chunk_size=30000, global_step=None, return_viz=False):
        ...
        if self.enable_quadtree:
            with torch.no_grad():
                ...
                patch_sizes = self.quadtree_patch_sizes
                base_threshold = self.quadtree_base_threshold
                relax_factor = self.quadtree_relax_factor
                max_patch_size = max(patch_sizes)

                # 对齐到最大四叉树块大小，避免边缘处 repeat 后尺寸不足。
                pad_h = (max_patch_size - H % max_patch_size) % max_patch_size
                pad_w = (max_patch_size - W % max_patch_size) % max_patch_size
        ...
        with torch.no_grad():
            scale_map_flat = scale_map.detach().float().squeeze(-1).reshape(B, -1)
            keep_mask_flat = keep_mask.detach().float().reshape(B, -1)
            quadtree_stats = {
                "quadtree_support_density": keep_mask_flat.mean(dim=1),
                "quadtree_scale_mean": scale_map_flat.mean(dim=1),
                "quadtree_base_threshold": torch.full(
                    (B,),
                    float(self.quadtree_base_threshold),
                    device=imgs.device,
                    dtype=torch.float32,
                ),
                "quadtree_relax_factor": torch.full(
                    (B,),
                    float(self.quadtree_relax_factor),
                    device=imgs.device,
                    dtype=torch.float32,
                ),
            }
            for scale_value in (1, 2, 4, 8, 16, 32):
                quadtree_stats[f"quadtree_scale_frac_{scale_value}"] = (
                    scale_map_flat == float(scale_value)
                ).float().mean(dim=1)
        ...
        if return_viz:
            viz_outputs = {
                ...
                "quadtree_base_threshold": imgs.new_tensor(self.quadtree_base_threshold),
                "quadtree_relax_factor": imgs.new_tensor(self.quadtree_relax_factor),
            }
