...
class Pi3LossGS(nn.Module):
    def __init__(
        ...
        self.norm_strategy = str(norm_strategy).lower()
        if self.norm_strategy not in ("mean", "median", "trimmed_mean"):
            raise ValueError(f"Unsupported norm_strategy: {norm_strategy}")
        self.use_scheduled_weights = bool(use_scheduled_weights)
    ...
    @staticmethod
    def _quantile_or_value(values, q):
        values = values.detach().float().reshape(-1)
        if values.numel() > 65536:
            stride = max(1, values.numel() // 65536)
            values = values[::stride][:65536]
        if values.numel() <= 1:
    ...
    def normalize_pred(self, pred, gt):
        ...
        norm_float = norm_factor.detach().float()
        norm_stat = torch.cat(norm_stat_chunks) if norm_stat_chunks else norm_float
        pred['_loss_norm_stats'] = {
            "norm_factor_mean": norm_stat.mean(),
            "norm_factor_median": self._quantile_or_value(norm_stat, 0.5),
            "norm_factor_p90": self._quantile_or_value(norm_stat, 0.9),
            "norm_factor_used_mean": norm_float.mean(),
            "norm_factor_used_median": self._quantile_or_value(norm_float, 0.5),
            "norm_factor_used_p90": self._quantile_or_value(norm_float, 0.9),
        }
        return pred
    ...
    def forward(self, pred, gt_raw, batch_idx=0, current_epoch=None, total_epochs=None, **kwargs):
        ...
        norm_stats = pred.get('_loss_norm_stats', {})
        # ==========================================
        ...
        details.update({
            ...
            "norm_factor_median": norm_stats.get(
                "norm_factor_median",
                torch.tensor(0.0, device=pred_c2w.device),
            ),
            "norm_factor_p90": norm_stats.get(
                "norm_factor_p90",
                torch.tensor(0.0, device=pred_c2w.device),
            ),
            "norm_factor_used_mean": norm_stats.get(
                "norm_factor_used_mean",
                torch.tensor(0.0, device=pred_c2w.device),
            ),
            "norm_factor_used_median": norm_stats.get(
                "norm_factor_used_median",
                torch.tensor(0.0, device=pred_c2w.device),
            ),
            "norm_factor_used_p90": norm_stats.get(
            ...
            "cur_weight_lpips": torch.tensor(float(weight_lpips), device=pred_c2w.device),
            "scheduled_weight_rgb": torch.tensor(float(cur_lambda_rgb), device=pred_c2w.device),
            "scheduled_weight_ssim": torch.tensor(float(cur_lambda_ssim), device=pred_c2w.device),
            "scheduled_weight_depth": torch.tensor(float(cur_lambda_depth), device=pred_c2w.device),
            "scheduled_weight_lpips": torch.tensor(float(cur_lambda_lpips), device=pred_c2w.device),
            "use_scheduled_weights": torch.tensor(
                1.0 if self.use_scheduled_weights else 0.0,
                device=pred_c2w.device,
            ),
            "cur_weight_edge": torch.tensor(self.lambda_edge, device=pred_c2w.device),
