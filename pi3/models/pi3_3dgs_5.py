...
class GlobalGaussianSlotEmitter(nn.Module):
    def __init__(
        self,
        token_dim,
        num_slots=8192,
        ...
        xyz_residual_ratio=0.03,
        scale_min_ratio=2e-4,
        scale_max_ratio=0.04,
        color_residual_strength=0.25,
        slot_fourier_bands=16,
        slot_existence_bias=-1.5,
    ):
        super().__init__()
        self.num_slots = max(1, int(num_slots))
        self.slot_dim = int(slot_dim)
        self.attention_chunk_size = max(1, int(attention_chunk_size))
        self.xyz_residual_ratio = max(0.0, float(xyz_residual_ratio))
        self.scale_min_ratio = max(0.0, float(scale_min_ratio))
        self.scale_max_ratio = max(float(scale_max_ratio), self.scale_min_ratio + 1e-6)
        self.color_residual_strength = max(0.0, float(color_residual_strength))
        self.slot_fourier_bands = max(1, int(slot_fourier_bands))
        self.slot_existence_bias = float(slot_existence_bias)

        self.slot_seed = nn.Parameter(torch.randn(1, self.slot_dim) * 0.02)
        slot_coord_dim = 1 + 2 * self.slot_fourier_bands
        self.slot_coord_mlp = nn.Sequential(
            nn.Linear(slot_coord_dim, self.slot_dim),
            nn.GELU(),
            nn.Linear(self.slot_dim, self.slot_dim),
        )
        self.token_proj = nn.Sequential(
            nn.LayerNorm(token_dim),
            nn.Linear(token_dim, self.slot_dim),
            nn.GELU(),
            nn.Linear(self.slot_dim, self.slot_dim),
        )
        self.key_proj = nn.Linear(self.slot_dim, self.slot_dim)
        self.value_proj = nn.Linear(self.slot_dim, self.slot_dim)
        ...
        self.head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(self.slot_dim * 2, 18),
        )

    def _make_slots(self, slot_count, device, dtype):
        slot_count = max(1, int(slot_count))
        coord = torch.linspace(0.0, 1.0, slot_count, device=device, dtype=torch.float32).unsqueeze(-1)
        freqs = torch.pow(
            torch.tensor(2.0, device=device, dtype=torch.float32),
            torch.arange(self.slot_fourier_bands, device=device, dtype=torch.float32),
        ).view(1, -1)
        angles = coord * freqs * (2.0 * math.pi)
        slot_feat = torch.cat([coord, torch.sin(angles), torch.cos(angles)], dim=-1)
        slots = self.slot_coord_mlp(slot_feat).to(dtype=dtype)
        return slots + self.slot_seed.to(device=device, dtype=dtype)

    def _sample_dense_anchor(
        self,
        attn,
        raw_uv,
        local_pts_views,
        ...
        B, slot_count, _ = attn.shape
        N = local_pts_views.shape[1]
        H, W = local_pts_views.shape[2:4]
        patch_h, patch_w = patch_hw
        dtype = attn.dtype
        device = attn.device

        attn_view = attn.reshape(B, slot_count, N, patch_h, patch_w)
        view_weight = attn_view.sum(dim=(-1, -2))
        view_prob = view_weight / view_weight.sum(dim=-1, keepdim=True).clamp_min(1e-6)

        ys = torch.linspace(-1.0, 1.0, patch_h, device=device, dtype=dtype)
        xs = torch.linspace(-1.0, 1.0, patch_w, device=device, dtype=dtype)
        grid_y, grid_x = torch.meshgrid(ys, xs, indexing="ij")
        patch_grid = torch.stack([grid_x, grid_y], dim=-1)
        weighted_grid = (attn_view[..., None] * patch_grid.view(1, 1, 1, patch_h, patch_w, 2)).sum(dim=(-3, -2))
        grid = weighted_grid / view_weight.clamp_min(1e-6)[..., None]

        patch_step = raw_uv.new_tensor([
            2.0 / max(float(patch_w), 1.0),
            2.0 / max(float(patch_h), 1.0),
        ])
        grid = (grid + torch.tanh(raw_uv)[..., None, :] * patch_step).clamp(-1.0, 1.0)

        sample_grid = grid.permute(0, 2, 1, 3).reshape(B * N, slot_count, 1, 2)

        local_in = local_pts_views.permute(0, 1, 4, 2, 3).reshape(B * N, 3, H, W).float()
        color_in = color_views.reshape(B * N, 3, H, W).float()
        conf_in = conf_views.permute(0, 1, 4, 2, 3).reshape(B * N, 1, H, W).float()

        local_sample = F.grid_sample(
            local_in,
            sample_grid.float(),
            mode="bilinear",
            padding_mode="border",
            align_corners=True,
        ).reshape(B, N, 3, slot_count).permute(0, 3, 1, 2).to(dtype=dtype)
        color_sample = F.grid_sample(
            color_in,
            sample_grid.float(),
            mode="bilinear",
            padding_mode="border",
        ...
        conf_sample = F.grid_sample(
            conf_in,
            sample_grid.float(),
            mode="bilinear",
            padding_mode="border",
            align_corners=True,
        ).reshape(B, N, 1, slot_count).permute(0, 3, 1, 2).to(dtype=dtype)

        rot = camera_poses_views[:, None, :, :3, :3].to(dtype=dtype)
        trans = camera_poses_views[:, None, :, :3, 3].to(dtype=dtype)
        world_sample = torch.matmul(rot, local_sample.unsqueeze(-1)).squeeze(-1) + trans

        anchor_xyz = (view_prob[..., None] * world_sample).sum(dim=2)
        anchor_color = (view_prob[..., None] * color_sample).sum(dim=2)
        anchor_conf = (view_prob[..., None] * conf_sample).sum(dim=2)
        view_entropy = -(
            view_prob.float().clamp_min(1e-8) * view_prob.float().clamp_min(1e-8).log()
        ).sum(dim=-1) / math.log(max(N, 2))
        uv_offset_norm = torch.tanh(raw_uv).detach().float().norm(dim=-1)

        return anchor_xyz, anchor_color, anchor_conf, view_entropy, uv_offset_norm
    ...
    def forward(
        ...
        scene_size,
        slot_budget=None,
        local_pts_views=None,
        camera_poses_views=None,
        color_views=None,
        conf_views=None,
        patch_hw=None,
    ):
        B, T, _ = token_feats.shape
        device, dtype = token_feats.device, token_feats.dtype
        slot_count = self.num_slots if slot_budget is None else max(1, int(slot_budget))
        scene_size = scene_size.to(device=device, dtype=dtype).reshape(B).clamp_min(1e-6)
        token_xyz = token_xyz.to(device=device, dtype=dtype)
        token_color = token_color.to(device=device, dtype=dtype)
        token_conf = token_conf.to(device=device, dtype=dtype)

        token_xyz_norm = token_xyz / scene_size.view(B, 1, 1)
        token_input = torch.cat([token_feats, token_xyz_norm, token_color, token_conf], dim=-1)
        token_lat = self.token_proj(token_input)
        keys = F.normalize(self.key_proj(token_lat), dim=-1)
        values = self.value_proj(token_lat)
        slots = self._make_slots(slot_count, device, dtype).unsqueeze(0).expand(B, -1, -1)

        xyz_parts, rot_parts, scale_parts, opacity_parts, color_parts, conf_parts = [], [], [], [], [], []
        existence_parts = []
        entropy_sum = token_feats.new_tensor(0.0)
        view_entropy_sum = token_feats.new_tensor(0.0)
        uv_offset_sum = token_feats.new_tensor(0.0)
        entropy_num = 0
        view_entropy_num = 0
        uv_offset_num = 0
        inv_sqrt = 1.0 / math.sqrt(float(self.slot_dim))

        for start in range(0, slot_count, self.attention_chunk_size):
            end = min(start + self.attention_chunk_size, slot_count)
            q = self.slot_norm(slots[:, start:end])
            logits = torch.matmul(q, keys.transpose(-2, -1)) * inv_sqrt
            attn = torch.softmax(logits.float(), dim=-1).to(dtype=dtype)

            pooled = torch.matmul(attn, values)
            anchor_xyz = torch.matmul(attn, token_xyz)
            anchor_color = torch.matmul(attn, token_color)
            anchor_conf = torch.matmul(attn, token_conf)

            slot_state = q + pooled
            slot_state = slot_state + self.slot_ffn(slot_state)
            raw = self.head(slot_state)

            if (
                local_pts_views is not None
                and camera_poses_views is not None
                and color_views is not None
                and conf_views is not None
                and patch_hw is not None
            ):
                anchor_xyz, anchor_color, anchor_conf, view_entropy, uv_offset_norm = self._sample_dense_anchor(
                    attn,
                    raw[..., 15:17],
                    local_pts_views,
                    camera_poses_views,
                ...
                view_entropy_sum = view_entropy_sum + view_entropy.sum().to(dtype=dtype)
                view_entropy_num += view_entropy.numel()
                uv_offset_sum = uv_offset_sum + uv_offset_norm.sum().to(dtype=dtype)
                uv_offset_num += uv_offset_norm.numel()

            raw_rot = raw[..., 0:4].clone()
            raw_rot[..., 0] = raw_rot[..., 0] + 1.0
            rotation = F.normalize(raw_rot, dim=-1)

            scale_ratio = self.scale_min_ratio + torch.sigmoid(raw[..., 4:7]) * (
                self.scale_max_ratio - self.scale_min_ratio
            )
            scale = scale_ratio * scene_size.view(B, 1, 1)
            existence = torch.sigmoid(raw[..., 17:18] + self.slot_existence_bias)
            opacity = torch.sigmoid(raw[..., 7:8]) * existence
            color = (anchor_color + self.color_residual_strength * torch.tanh(raw[..., 8:11])).clamp(0.0, 1.0)
            xyz = anchor_xyz + torch.tanh(raw[..., 11:14]) * (
                scene_size.view(B, 1, 1) * self.xyz_residual_ratio
            )
            conf_logit = torch.logit(anchor_conf.clamp(1e-4, 1.0 - 1e-4)) + raw[..., 14:15]

            entropy = -(attn.float().clamp_min(1e-8) * attn.float().clamp_min(1e-8).log()).sum(dim=-1)
            entropy_sum = entropy_sum + (entropy / math.log(max(T, 2))).sum().to(dtype=dtype)
            entropy_num += entropy.numel()

            xyz_parts.append(xyz)
            rot_parts.append(rotation)
            scale_parts.append(scale)
            opacity_parts.append(opacity)
            color_parts.append(color)
            conf_parts.append(conf_logit)
            existence_parts.append(existence)

            del logits, attn, pooled, raw, slot_state

        gaussians = {
            "xyz": torch.cat(xyz_parts, dim=1),
            "rotation": torch.cat(rot_parts, dim=1),
            "scale": torch.cat(scale_parts, dim=1),
            "opacity": torch.cat(opacity_parts, dim=1),
            "color": torch.cat(color_parts, dim=1),
            "conf": torch.cat(conf_parts, dim=1),
            "existence": torch.cat(existence_parts, dim=1),
            "num_sky": 0,
        }
        existence = gaussians["existence"].detach()
        opacity = gaussians["opacity"].detach()
        stats = {
            "slot_count": torch.full((B,), float(slot_count), device=device, dtype=dtype),
            "slot_opacity_mean": opacity.mean(dim=(1, 2)).to(dtype=dtype),
            "slot_existence_mean": existence.mean(dim=(1, 2)).to(dtype=dtype),
            "slot_expected_active_count": existence.sum(dim=(1, 2)).to(dtype=dtype),
            "slot_existence_active_count": (existence.squeeze(-1) > 0.5).float().sum(dim=1).to(dtype=dtype),
            "slot_opacity_active_count": (opacity.squeeze(-1) > 0.03).float().sum(dim=1).to(dtype=dtype),
            "slot_attention_entropy": torch.full(
                (B,),
                float((entropy_sum / max(entropy_num, 1)).detach().float().item()),
                device=device,
                dtype=dtype,
            ),
            "slot_view_entropy": torch.full(
                (B,),
                float((view_entropy_sum / max(view_entropy_num, 1)).detach().float().item()),
                device=device,
                dtype=dtype,
            ),
            "slot_uv_offset": torch.full(
                (B,),
                float((uv_offset_sum / max(uv_offset_num, 1)).detach().float().item()),
                device=device,
                dtype=dtype,
            ),
        }
...
class Pi3_3DGS(nn.Module):
    def __init__(
            ...
            enable_slot_emitter=True,
            num_gaussian_slots=524288,
            slot_budget_mode="candidate_linear_coverage",
            slot_base_gaussian_slots=65536,
            slot_min_gaussian_slots=32768,
            slot_budget_ref_views=4,
            slot_budget_view_power=1.0,
            slot_budget_coverage_weight=0.5,
            slot_budget_multiple=256,
            slot_dim=256,
            slot_attention_chunk_size=1024,
            slot_xyz_residual_ratio=0.03,
            slot_scale_min_ratio=2e-4,
            slot_scale_max_ratio=0.04,
            slot_color_residual_strength=0.25,
            slot_fourier_bands=16,
            slot_existence_bias=-1.5,
    ):
        super().__init__()
        ...
        self.enable_slot_emitter = bool(enable_slot_emitter)
        self.num_gaussian_slots = max(1, int(num_gaussian_slots))
        self.slot_budget_mode = str(slot_budget_mode)
        self.slot_base_gaussian_slots = max(1, int(slot_base_gaussian_slots))
        self.slot_min_gaussian_slots = max(1, int(slot_min_gaussian_slots))
        self.slot_budget_ref_views = max(1.0, float(slot_budget_ref_views))
        self.slot_budget_view_power = max(0.0, float(slot_budget_view_power))
        self.slot_budget_coverage_weight = max(0.0, float(slot_budget_coverage_weight))
        self.slot_budget_multiple = max(1, int(slot_budget_multiple))
        self.slot_attention_chunk_size = max(1, int(slot_attention_chunk_size))
        ...
        slot_token_dim = 2 * self.dec_embed_dim + 7
        self.slot_emitter = GlobalGaussianSlotEmitter(
            token_dim=slot_token_dim,
            num_slots=self.num_gaussian_slots,
            slot_dim=slot_dim,
            attention_chunk_size=self.slot_attention_chunk_size,
            xyz_residual_ratio=slot_xyz_residual_ratio,
            scale_min_ratio=slot_scale_min_ratio,
            scale_max_ratio=slot_scale_max_ratio,
            color_residual_strength=slot_color_residual_strength,
            slot_fourier_bands=slot_fourier_bands,
            slot_existence_bias=slot_existence_bias,
        )
    ...
    def _load_state_dict_flexible(self, checkpoint):
        ...
        if skipped:
            ...
            print(f"[Pi3_3DGS_5] Skipped incompatible checkpoint tensors: {preview}{suffix}")
        return res
    ...
    def _compute_slot_budget(self, camera_poses_sub, scene_size):
        default_slots = int(self.num_gaussian_slots)
        if default_slots <= 0:
            return 1
        if self.slot_budget_mode.lower() in ("fixed", "constant", "max"):
            return default_slots

        B, N, _, _ = camera_poses_sub.shape
        device, dtype = camera_poses_sub.device, camera_poses_sub.dtype
        with torch.no_grad():
            scene = self._normalize_scene_size(scene_size, B, device, dtype)
            centers = camera_poses_sub[..., :3, 3].detach().float()
            span = (centers.amax(dim=1) - centers.amin(dim=1)).norm(dim=-1)
            coverage = (span / scene.detach().float().clamp_min(1e-6)).mean().clamp_min(0.0).item()

        mode = self.slot_budget_mode.lower()
        view_ratio = float(N) / self.slot_budget_ref_views
        if mode in ("power", "power_coverage", "sublinear", "coverage_power"):
            view_factor = max(1.0, view_ratio ** self.slot_budget_view_power)
        else:
            # Default behavior: capacity grows at least linearly with the number of selected views.
            # Overlap/redundancy should be learned by slot opacity and collapse regularization,
            # not imposed by shrinking the per-view Gaussian budget as N increases.
            view_factor = max(1.0, view_ratio)

        coverage_factor = 1.0 + self.slot_budget_coverage_weight * coverage
        budget = int(round(self.slot_base_gaussian_slots * view_factor * coverage_factor))
        budget = max(self.slot_min_gaussian_slots, budget)
        if self.slot_budget_multiple > 1:
            budget = ((budget + self.slot_budget_multiple - 1) // self.slot_budget_multiple) * self.slot_budget_multiple
        return max(1, budget)
    ...
    def forward(self, imgs, intrinsics=None, chunk_size=30000, global_step=None, return_viz=False):
        ...
        K[:, :, 1, 2] = cy
        K[:, :, 2, 2] = 1.0
        # Slot version intentionally keeps the original Pi3-style intrinsics
        # estimated from predicted xy. The intrinsics argument is accepted only
        # for trainer API compatibility.
        # ==========================================================
        ...
        if self.enable_slot_emitter:
            ...
            token_feats = hidden_views[:, :, self.patch_start_idx:].reshape(B, N_sub, patch_h, patch_w, -1)
            slot_budget = self._compute_slot_budget(camera_poses_sub, scene_size)

            color_views = imgs_raw[:, sub_idx]
            conf_views = torch.sigmoid(conf_logits_sub)
            grid_y, grid_x = torch.meshgrid(
                torch.linspace(-1.0, 1.0, patch_h, device=imgs.device, dtype=imgs.dtype),
                torch.linspace(-1.0, 1.0, patch_w, device=imgs.device, dtype=imgs.dtype),
                indexing="ij",
            )
            patch_center_grid = torch.stack([grid_x, grid_y], dim=-1)
            patch_center_grid = patch_center_grid.view(1, patch_h, patch_w, 2).expand(B * N_sub, -1, -1, -1)

            local_patch = F.grid_sample(
                local_pts_sub.permute(0, 1, 4, 2, 3).reshape(B * N_sub, 3, H, W),
                patch_center_grid,
                mode="bilinear",
                padding_mode="border",
                align_corners=True,
            ).reshape(B, N_sub, 3, patch_h, patch_w).permute(0, 1, 3, 4, 2)
            color_patch = F.grid_sample(
                color_views.reshape(B * N_sub, 3, H, W),
                patch_center_grid,
                mode="bilinear",
                padding_mode="border",
                align_corners=True,
            ).reshape(B, N_sub, 3, patch_h, patch_w).permute(0, 1, 3, 4, 2)
            conf_patch = F.grid_sample(
                conf_views.permute(0, 1, 4, 2, 3).reshape(B * N_sub, 1, H, W),
                patch_center_grid,
                mode="bilinear",
                padding_mode="border",
                align_corners=True,
            ).reshape(B, N_sub, 1, patch_h, patch_w).permute(0, 1, 3, 4, 2)

            cam_rot_patch = camera_poses_sub[:, :, None, None, :3, :3]
            cam_trans_patch = camera_poses_sub[:, :, None, None, :3, 3]
            world_patch = torch.matmul(cam_rot_patch, local_patch.unsqueeze(-1)).squeeze(-1) + cam_trans_patch

            token_feats_flat = token_feats.reshape(B, N_sub * patch_h * patch_w, -1)
            world_patch_flat = world_patch.reshape(B, N_sub * patch_h * patch_w, 3)
            color_patch_flat = color_patch.reshape(B, N_sub * patch_h * patch_w, 3)
            conf_patch_flat = conf_patch.reshape(B, N_sub * patch_h * patch_w, 1)

            gaussians, slot_stats = self.slot_emitter(
                token_feats_flat,
                world_patch_flat,
                color_patch_flat,
                conf_patch_flat,
                ...
                local_pts_views=local_pts_sub,
                camera_poses_views=camera_poses_sub,
                color_views=color_views,
                conf_views=conf_views,
                patch_hw=(patch_h, patch_w),
            )
            zero_stat = scene_size.new_zeros(B)
            gaussian_stats = {
                "count_before": slot_stats["slot_count"],
                "count_after": slot_stats["slot_count"],
                "pruned": zero_stat,
                "candidates": zero_stat,
                "threshold": zero_stat,
                "voxel_size": zero_stat,
                "mean_redundancy": zero_stat,
                **slot_stats,
            }
            ...
            if return_viz:
                output["routing_viz"] = {
                    ...
                    "slot_anchor_points": world_patch.detach().clone(),
                    "slot_anchor_conf": conf_patch.detach().squeeze(-1).clone(),
        ...
        #####
            ...
            if len(b_xyz) > 0:
                ...
                gaussian_b, stats_b = self._apply_redundancy_mercy(gaussian_b, scene_size[b])
